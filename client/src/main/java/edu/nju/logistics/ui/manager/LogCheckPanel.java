...
public class LogCheckPanel extends JPanel implements MouseListener {

	//private static final ImageIcon CHECK = new ImageIcon("Image/manager/1.png");
	/**
	 * 显示区宽度
	 */
	...
	private commonButton check_lab = null;
	...
	public LogCheckPanel(OperationManagementBLService controller,DisconnectInformer disconnectInformer) {
	...
	private void decorateTable(){
		...
		this.table.getColumnModel().getColumn(2).setPreferredWidth(710);
	...
	private void checkLog() throws RemoteException {
		...
		if (!this.model.showByDate(date, this.controller)) {
			GlobalHintInserter.insertGlobalHint("该日期无日志查询！");
