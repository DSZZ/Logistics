...
public class BalanceChartPanel extends JPanel {
	...
	private BalanceChartVO chartVO = null;

	private MyThread thread;

	private MyAnotherThread thread1;
	// 收入
	...
	public BalanceChartPanel(OperationManagementBLService controller, DisconnectInformer disconnectInformer) {
		try {
			this.setSize(980, 560);
			this.setLayout(null);
			this.setOpaque(false);
			this.controller = controller;
			this.chartVO = controller.getBalanceChar();
			...
			// this.balance = 10000;
			//
			// this.totalFreight = 8000;
			// this.totalRent = 5000;
			// this.totalSalary = 2000;
			// System.out.println(this.income+" "+this.cost+" "+this.balance+"
			// "+this.totalRent+" "+this.totalFreight+" "+this.totalSalary);
	...
	class MyThread extends Thread {
		...
		public void run() {
			...
			w2 = 0;
			w3 = 0;
			balanceVLB.setVisible(false);
			costVLB.setVisible(false);
			incomeVLB.setVisible(false);
	...
	class MyAnotherThread extends Thread {
		...
		public void run() {
			...
			a2 = 0;
			a3 = 0;
			freightVLB.setVisible(false);
			rentVLB.setVisible(false);
			salaryVLB.setVisible(false);
